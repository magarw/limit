...
# paths and constants
CUTOFF = 100
INPUT_PATH = f"../../../data/clean/african-storybooks-initiative/non-sparse/cutoff-{CUTOFF}/"
OUTPUT_PATH = "../../../data/clean/african-storybooks-initiative/eng-mismatch-removed/"
...
for file_name in input_filenames:
    ...
    if "english" not in json_file["language"].lower():
        text = [x['text'] for x in json_file["pages"]]
        ...
        for t in text:
            ...
            for p in prediction:
                if p.lang == 'en' and p.prob > 0.90:
                    ...
                    #print(f"Too much english p={p.prob}, file={file_name}")
        if num_pages_with_excess_english/len(text) >= 0.5: # i.e. more than half the pages have excess English
            with open(f"{OUTPUT_PATH}candidate_removals_cutoff{CUTOFF}.txt", "a") as f:
                f.write(f"Excess: {num_pages_with_excess_english}/{len(text)}; File: {file_name}; OrigLang: {json_file['language']}\n")
